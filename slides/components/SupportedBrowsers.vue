...
const items = [
	{
		name: 'chrome',
		wixSupport: browsers.chrome,
		version: props.chrome || '-',
		isSupported: (parseFloat(props.chrome ?? '0') || Infinity) <= browsers.chrome
	},
	{
		name: 'edge',
		wixSupport: browsers.edge,
		version: props.edge || '-',
		isSupported: (parseFloat(props.edge ?? '0') || Infinity) <= browsers.edge
	},
	{
		name: 'firefox',
		wixSupport: browsers.firefox,
		version: props.firefox || '-',
		isSupported: (parseFloat(props.firefox ?? '0') || Infinity) <= browsers.firefox
	},
	{
		name: 'safari',
		wixSupport: browsers.safari,
		version: props.safari || '-',
		isSupported: (parseFloat(props.safari ?? '0') || Infinity) <= browsers.safari
	}
];
</script>
...
<template>
	...
	>
		<span
			v-for="item in items"
      :title="`Wix supports: ${item.wixSupport}`"
			...
			:class="{ browser: true, [`${item.name}`]: true, supported: item.isSupported }"
			role="img"
			>{{ item.version }}</span
		>
	</a>
